    ...
    while running:
        ...
        with open(latest_log, "rb") as f:
            ...
            while running and dpg.is_dearpygui_running():
                ...
                # Heartbeat pulsing
                if dpg.does_item_exist("heartbeat"):
                    hb = 4 if stats["under_attack"] else 2
                    dpg.set_value("heartbeat", "[*]" if int(now * (hb/2)) % 2 == 0 else "[ ]")
                ...
                # --- INSERT v2.7 BLOCK HERE ---
                if "deactivates because its target" in text:
                    if not stats["paused"]:
                        stats["history"].insert(0, f"[{ts}] !! ROCK POPPED !!")
                        # Reset idle timer to force immediate Red "IDLE" state
                        stats["last_hit_time"] = now - IDLE_THRESHOLD 

                if "jammed by" in text:
                    stats["under_attack"] = True
                    stats["last_combat_time"] = now
                    stats["history"].insert(0, f"[{ts}] !! ECM JAM DETECTED !!")
                # -------------------------------

                # Logic Check for Cargo Full
                ...
                # Combat/Mining Parsing
                is_combat = (" from " in text or " to " in text) and any(res in text for res in ["Hits", "Smashes", "Glances", "Penetrates"])
                if is_combat: stats["under_attack"] = True; stats["last_combat_time"] = now
                ...
                if "Critical mining success" in text and crit_m:
                    val = int(crit_m.group(1).replace(',', ''))
                    stats["total"] += val; stats["last_crit"] = val; stats["last_hit_time"] = now
                    stats["history"].insert(0, f"[{ts}] *** CRIT: +{val:,} ***")
                elif "You mined" in text and norm_m:
...
# --- GUI ---
dpg.create_context()
with dpg.window(label="MINING OPS CONSOLE V2.6.2", tag="PrimaryWindow"):
    with dpg.group(horizontal=True):
        dpg.add_text("[ ]", tag="heartbeat", color=[0, 255, 255])
...
dpg.create_viewport(title='SENTRY_V2.6.2', width=360, height=520)
